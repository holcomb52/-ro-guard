...
SUPABASE_URL = "https://eyufnhnabdgekhfvhqzf.supabase.com"
SUPABASE_KEY = "sb_publishable_5SXVN_0B5aIouuZA0a3b3Q_Mq4chxUT"
...
def save_learned_claims(file_name, claims):
    ...
    for idx, claim in enumerate(claims, start=1):
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (datetime.now().isoformat(timespec="seconds"), file_name, idx, claim))
    conn.commit()
    conn.close()
